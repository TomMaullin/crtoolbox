...
fsl_sub -l $CONFSETS_PATH/results/sim$simNo/log/ -N genCfgs -q long.qf bash $CONFSETS_PATH/genCfgs_Mmu.sh \
...
for cfg in $CONFSETS_PATH/results/sim$simNo/cfgs/cfg*.yml; do
	...
	bash $CONFSETS_PATH/SpatialSims_Mmu.sh $cfg \
...
# Submit the job
fsl_sub -j $simInstancesID -l $CONFSETS_PATH/results/sim$simNo/log/ -N concat -q long.qf \
bash $CONFSETS_PATH/join_and_plot_Mmu.sh $CONFSETS_PATH/results $simNo > \
